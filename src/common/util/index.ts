...
export const generateSlug = async (identifier: string, model: any, language: LanguageEnum | LanguageEnumCMS): Promise<string> => {
...
export function findMostMatchedString(str1, data) {
    ...
    const d = matches.reduce((maxItem, currentItem) => (currentItem.count > maxItem.count ? currentItem : maxItem), { count: -1 });
